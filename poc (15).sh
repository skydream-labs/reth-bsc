...
# [8b+] steady-state finality gate — THE enforcement point of the attack.
# The forge gate (justified == head-1) only opens once fast finality catches up
# with head. Fresh machine: the first run BUILDS the reth image (~20-60m) WHILE
# the chain is running -> CPU contention -> slow vote propagation -> finality
# lags 2-3 blocks (observed: lag=3, delta=0, gate never opened). Wait until
# lag<=1 + control parity, max 600s.
SS_OK=0; SS_LAG=99; SS_H=0; SS_F=0; SS_CF=0
for i in $(seq 1 300); do
  SS_H=$(jnum "$VICTIM_RPC" latest); SS_F=$(jnum "$VICTIM_RPC" finalized); SS_CF=$(jnum "$CRPC" finalized)
  if [ "$SS_H" -ge 0 ] 2>/dev/null && [ "$SS_F" -ge 0 ] 2>/dev/null; then
    SS_LAG=$(( SS_H - SS_F ))
    if [ "$SS_LAG" -le 1 ] && [ "$SS_F" = "$SS_CF" ]; then SS_OK=1; break; fi
  fi
  sleep 2
done
if [ "$SS_OK" != "1" ]; then
  echo "FATAL: fast finality not steady yet (lag=$SS_LAG, need <=1) after 600s."
  echo "  Common cause: first run on a fresh machine (image build running alongside the chain)"
  echo "  slows validator vote propagation. Fix: re-run ./poc.sh — the build is now cached and"
  echo "  finality will reach lag<=1."
  exit 1
fi
BF_V=$SS_F; BF_C=$SS_CF; VV=$SS_H
echo "    [8b+] steady state OK: head=$SS_H finalized=$SS_F (lag=$SS_LAG) control=$SS_CF"
